...
template <typename T>
struct GenerateCurvatureNormals {
  const T* Points;
  ...
  // local storage lots of new/delete.
  vtkSMPThreadLocalObject<vtkIdList> PIds;

  GenerateCurvatureNormals(T* points, vtkAbstractPointLocator* loc, int sample,
                           float* curvature, float* normals, int orient,
  ...
  static void Execute(vtkOrthodonticsPCACurvatureNormalEstimation* self,
                      vtkIdType numPts, T* points, float* curvature,
                      float* normals, int orient, double opoint[3], bool flip) {
    GenerateCurvatureNormals gen(points, self->GetLocator(),
                                 self->GetSampleSize(), curvature, normals,
                                 orient, opoint, flip);
    vtkSMPTools::For(0, numPts, gen);
  }
};  // GenerateCurvatureNormals
}  // anonymous namespace

vtkStandardNewMacro(vtkOrthodonticsPCACurvatureNormalEstimation);

// Produce the output data
int vtkOrthodonticsPCACurvatureNormalEstimation::RequestData(
  ...
  switch (input->GetPoints()->GetDataType()) {
    vtkTemplateMacro(GenerateCurvatureNormals<VTK_TT>::Execute(
        this, numPts, (VTK_TT*)inPtr, c, n, this->NormalOrientation,
        this->OrientationPoint, this->FlipNormals));
  }
  // Orient the normals in a consistent fashion (if requested). This requires a
  // traversal
